...
set -euo pipefail
# SELF-REWRITE SAFETY (2026-08-25): launchd runs this file straight out of the
# repo (the daemon plist points at an absolute path in ops/), and the very first
# thing it does is `git pull` - which can rewrite THIS FILE while bash is still
# reading it. Bash reads a script incrementally by byte offset, so an in-place
# rewrite can leave it resuming at whatever now sits at that offset, executing a
# fragment of a line. The MacBook hit exactly this on 2026-08-25 via `cp` over
# its live auto-sync.sh ("line 23: om.gostork.nightly-sync: command not found").
# Here it would run as ROOT. Wrapping the body in a function forces bash to
# parse the whole thing up front, so the file on disk can change freely once
# main() is running. Keep `main "$@"` as the last line and add nothing after it.
main() {
