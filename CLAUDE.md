...
- **AI Prompt Seeding - CRITICAL ARCHITECTURE RULE:** The AI concierge system prompt uses a two-layer system. Layer 1: `ConciergePromptSection` DB table - the DB version always wins at runtime. The admin page at `/account/concierge` shows and edits these directly. Layer 2: `server/ai-prompt-defaults.ts` - the code source of truth. On server restart, `server/index.ts` seeds ONLY sections that do not yet exist in DB (never overwrites). **MANDATORY WORKFLOW FOR ALL PROMPT CHANGES: (1) update `server/ai-prompt-defaults.ts`, AND (2) immediately push to DB via Supabase MCP `execute_sql` UPDATE on `ConciergePromptSection` where key = the section key. Project ID: bryzqwfzvgjenijciwaa. Both steps required every time - file keeps code in sync, DB push makes it live on the admin page immediately.** Never put prompt logic in the hardcoded `biologicalMasterLogic` fallback in `ai-router.ts` - dead code when DB sections exist. Dynamic skip directives (per-request, from chat history + user profile) live in `ai-router.ts` lines ~1508-1600 - always applied on top of DB prompt.
