...
export async function cloudflareSyncStatus(): Promise<SyncResult> {
  ...
  try {
    ...
    const rule = (entry?.result?.rules || []).find((r: any) => r.description === RULE_MARKER);
    if (!rule) return { ok: true, configured: true, message: "Connected. No edge rule yet - run a sync." };
    const m = String(rule.expression || "").match(/"[A-Z]{2}"/g);
    return {
      ok: true,
      configured: true,
      message: `Edge rule active, blocking ${m?.length ?? 0} countries.`,
...
export async function syncBlockedCountriesToCloudflare(): Promise<SyncResult> {
  ...
  try {
    ...
    const existing = (entry?.result?.rules || []).find((r: any) => r.description === RULE_MARKER);

    // No blocked countries: disable our rule rather than deleting it, so the
    // history of its existence survives in the dashboard.
    if (codes.length === 0) {
      if (existing) {
        await cf(`/zones/${c.zoneId}/rulesets/${rulesetId}/rules/${existing.id}`, c.token, {
          method: "PATCH",
    ...
    if (existing) {
      ...
      return { ok: true, configured: true, message: `Edge rule updated - blocking ${codes.length} countries.`, blockedCount: codes.length, ruleId: existing.id };
    }
