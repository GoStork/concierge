...
4. If the search returns none of these agencies (not on GoStork or filtered out), do NOT substitute a different agency. Explain that ${ce.clinicName} works only with its own partner agencies and warmly suggest the parent ask the clinic directly about them - or, if the parent prefers an independent agency choice, note that it may mean working with a different clinic.`);
      ...
      if (upcomingProviderConsult) {
        ...
        if (prepMissing.length > 0) {
          parts.push(`CALL PREP MODE - ACTIVE: The parent's consultation call is ALREADY BOOKED and confirmed - do NOT offer the calendar again, do NOT emit [[CONSULTATION_BOOKING]]${partnerClinicPending ? " (EXCEPTION: you MAY and SHOULD still book the partner IVF clinic named in the PARTNER IVF CLINIC PENDING directive above - offer that clinic call before starting prep)" : ""}, do NOT ask about scheduling or time preferences. Follow the POST-BOOKING CALL PREP section. Items still missing for the provider: ${prepMissing.map((m, i) => `(${i + 1}) ${m}`).join("; ")}. THIS LIST IS A SNAPSHOT taken BEFORE the parent's current message: if their current message answers the first listed item (e.g. the list says partner name/age and they just wrote "Maria, 34"), save it with [[SAVE:...]], acknowledge in a few words, and ask the item AFTER it - NEVER ask a question the message you are replying to just answered. Otherwise, when the parent agrees to prep (e.g. "Let's do it") or sends any message while prep is pending, ask the FIRST missing item - one question per message, IN THE ORDER LISTED, framed as preparing for their call. If their message contains a question or a request of its own, answer/handle THAT first in the same reply, then ask the next missing prep item - NEVER ignore what the parent actually said. THIS OVERRIDES THE FAVORITE FLOW: if the parent just favorited a profile, confirm the favorite in ONE sentence (note the agency can discuss her on the upcoming call - do NOT offer to schedule another call, do NOT ask if they have questions or want more profiles), then ask the next missing prep item in the SAME reply. NEVER refuse the save - the system already saved it when the heart was tapped; if the profile has a hard incompatibility (e.g. not open to single parents), keep the confirmation, add ONE short heads-up sentence about it, and STILL ask the next prep item. Everything else about the parent is already saved - do not re-ask it.`);
        }
      }
