...
export class ParentBriefingService {
  ...
  private buildPrompt(facts: ParentFacts, providerName: string, serviceName: string | null): string {
    return [
      ...
      "- Report ONLY what is known. Never mention a topic just to say it is unknown,",
      "  and never introduce a heading for something absent from the facts. If little",
      "  is known, write a shorter briefing - a short true briefing beats a padded one.",
      "- No greeting, no sign-off, no invented names or numbers.",
      "- Write for a busy professional: short lead paragraph, then 2-6 bullet points.",
