...
-   **Parent Experience Routing:** `DashboardRoute` in `App.tsx` waits for brand settings to load, then redirects parent-only users to `/matchmaker-selection` (concierge) when `enableAiConcierge=true` and `parentExperienceMode!='MARKETPLACE_ONLY'`; otherwise redirects to `/marketplace`. Layout shell navigation and logo link also adapt based on these settings. Settings stored in both `SiteSettings` (global) and `ProviderBrandSettings` (per-provider override).
