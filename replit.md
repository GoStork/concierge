...
-   **Chat History Page:** `/chat` route shows parent's conversation history with Eva concierge sessions and provider conversations. Lists sessions with matchmaker avatar, name, title, last message preview, and time ago. Supports session resumption — clicking a session navigates to `/concierge?matchmaker={id}&session={id}` which loads existing messages. API: `GET /api/my/chat-sessions`. `AiChatSession` stores `matchmakerId` to associate sessions with their concierge persona. Back button in concierge chat navigates to `/chat`.
