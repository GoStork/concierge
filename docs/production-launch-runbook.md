...
5. Step 5 (2026-08-19):
   ...
   b. [x] **Nightly-sync pinger rewired** (4bcd4a1a): `.github/workflows/
      ...
      [x] **GitHub cron is NOT a reliable driver** (found Sep 3 2026): the
      scheduled runs fired 10:38-12:42 UTC every day (4-6h late) and skipped
      Sep 3 entirely. **Fixed Sep 3 2026**: the prod VM now runs its own
      `gostork-nightly.timer` (06:00 + 07:00 UTC) that POSTs
      `http://127.0.0.1:5001/api/cron/run-nightly-sync` with the host's
      `NIGHTLY_SYNC_SECRET`; the app's 2 AM ET slot dedup makes the second
      tick and the GitHub ping no-ops. Source of truth: `ops/prod/` in the
      repo; re-install with `sudo bash /srv/gostork/app/ops/prod/
      install-nightly-timer.sh`. GitHub workflow kept as backup. Check:
      `systemctl list-timers | grep gostork-nightly`. At Phase B nothing
      changes here (it targets localhost).
      Agent access to the VM: service account
      `gostork-vm-ssh@gostork.iam.gserviceaccount.com` (OS Login admin +
      compute viewer; key at `~/.config/gostork/gostork-vm-ssh.json` on the
      MacBook, registered in gcloud) - `gcloud compute ssh --account
      gostork-vm-ssh@gostork.iam.gserviceaccount.com ...` never hits the
      Workspace reauth wall that expires the human login daily. [ ] ERAN: the repo secret
...
- `OTP_TEST_NUMBERS` (dev-only allowlist that skips OTP rate limits for listed
  phones) must NOT be set in the prod env. The guard ignores it whenever
  APP_URL is https://app.gostork.com (dev Macs also run NODE_ENV=production,
  so the host is the signal), but keep the variable out of prod anyway.
...
- [x] **LIVE KEYS INSTALLED 2026-08-19**: Eran revealed the live secret
  ...
  block shows Fields configured. **Template-level flag, not code (2026-09-11):**
...
**The trap.** A GoStork staff account that has never enrolled cannot complete a
password login once that date has passed - and enrolment happens *inside* the
app, after logging in. So the next person added to the team, or any account
promoted to a GOSTORK_* role before enrolling, is locked out with no self-serve
way back. This is not hypothetical: it broke four test fixtures the moment
