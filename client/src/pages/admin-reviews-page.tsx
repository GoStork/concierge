  ...
  return (
    <div className="max-w-[1100px] mx-auto w-full px-4 sm:px-6 py-6 space-y-5">
      <div>
        ...
        <p className="t-helper mt-1">Reviews auto-publish after AI screening. Remove anything that slipped through, restore anything removed in error, and follow up on private feedback.</p>
      </div>
      ...
      {q.isLoading ? (
        ...
        <p className="t-helper py-8">No reviews match this filter.</p>
      ) : (
            ...
            <div key={r.id} className="rounded-[var(--radius)] border bg-card p-4" data-testid={`admin-review-${r.id}`}>
              <div className="flex items-start justify-between gap-3 flex-wrap">
                <div className="space-y-1 min-w-0">
                  ...
                  {r.aiScreenNotes && <p className="t-helper">AI screening: {r.aiScreenNotes}</p>}
                  {r.providerReply && (
                    <div className="pl-3 border-l-2 border-primary/30">
                      <p className="t-helper">Provider reply: {r.providerReply}</p>
                    </div>
                  )}
                </div>
