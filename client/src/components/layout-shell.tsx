...
export function LayoutShell({ children }: { children: React.ReactNode }) {
  ...
  const handleIpFormPartnerSignedEvent = useCallback((data: any) => {
    ...
    playNotificationChime();
    const signer = data.signerName || "Your partner";
    const { id: toastId } = toast({
      title: `${signer} signed your Intended Parent Form`,
      description: "It's ready to review and submit.",
      variant: "success",
      action: (
        <Button
          size="sm"
          variant="default"
          className="gap-1 shrink-0"
          onClick={() => { dismiss(toastId); navigate("/ip-form"); }}
          data-testid="button-view-ip-form-from-toast"
        >
          Review & submit
        </Button>
      ),
    ...
    if ("Notification" in window && Notification.permission === "granted") {
      try {
        new Notification(`${signer} signed your Intended Parent Form`, { body: "It's ready to review and submit.", icon: "/favicon.ico", tag: `ip-form-signed-${data.responseId}` });
      } catch {}
    }
  ...
  const handleIpFormSentEvent = useCallback((data: any) => {
    if (data.type !== "ip_form_sent_to_parent") return;
    playNotificationChime();
    const parent = data.parentName || "A parent";
    const { id: toastId } = toast({
      title: `${parent} completed their consultation`,
      description: "Their Intended Parent Form has been sent - you'll get it here once submitted.",
      variant: "success",
      action: data.sessionId ? (
        <Button size="sm" variant="default" className="gap-1 shrink-0" onClick={() => { dismiss(toastId); navigate(`/chat/${data.sessionId}`); }} data-testid="button-view-ip-form-chat-from-toast">
          Open chat
        </Button>
      ) : undefined,
      duration: 20000,
