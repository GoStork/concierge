...
export function LayoutShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className={`pt-0 ${showCalendarBanner ? 'md:pt-[88px]' : 'md:pt-16'} ${location.pathname === '/marketplace' || /^\/(surrogate|eggdonor|spermdonor)\//.test(location.pathname) ? 'pb-0' : location.pathname.startsWith('/chat') || location.pathname.startsWith('/admin/concierge-monitor') ? 'pb-0' : 'pb-28'} md:pb-0 ${location.pathname.startsWith('/chat') || location.pathname.startsWith('/admin/concierge-monitor') ? 'h-dvh overflow-hidden' : 'min-h-screen'} transition-all duration-300`}>
