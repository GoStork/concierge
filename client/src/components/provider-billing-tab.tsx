...
function CombinedPreview({ services, configByService }: CombinedPreviewProps) {
  // One row per enabled service; sample dollar amount the admin can type to
  // see the resulting invoice line. Seeded from each service's
  // sampleTotalCostCents if available, else the defaultServiceAmount.
  const initialAmounts = useMemo(() => {
    ...
    services.forEach(st => {
      ...
      else if (cfg?.defaultServiceAmount) out[st] = String(cfg.defaultServiceAmount / 100);
      else out[st] = "";
    });
    return out;
  }, [services, configByService]);

  const [amounts, setAmounts] = useState<Record<LineServiceType, string>>(initialAmounts);
  useEffect(() => setAmounts(initialAmounts), [initialAmounts]);

  // Per-line and total fee math (mirrors BillingService.computeFee).
  const lines = services.map(st => {
    const cfg = configByService.get(st);
    const lineAmountCents = Math.round((parseFloat(amounts[st] || "") || 0) * 100);
    if (!cfg) {
      return { serviceType: st, configured: false, lineAmountCents, feeCents: 0, payoutCents: lineAmountCents };
    }
    let feeCents = 0;
    if (cfg.feeType === "FLAT") {
      feeCents = Math.round(Number(cfg.flatAmount) || 0);
    } else if (cfg.feeType === "PERCENTAGE") {
      feeCents = Math.round(lineAmountCents * ((Number(cfg.percentage) || 0) / 100));
    }
    feeCents = Math.min(feeCents, lineAmountCents);
    return {
      serviceType: st,
      configured: true,
      cfg,
      lineAmountCents,
      feeCents,
      payoutCents: lineAmountCents - feeCents,
    } as const;
  });

  const parentPaysTotal = lines.reduce((s, l) => s + l.lineAmountCents, 0);
  const feeTotal = lines.reduce((s, l) => s + l.feeCents, 0);
  const payoutTotal = parentPaysTotal - feeTotal;

  const hasAnyAmount = parentPaysTotal > 0;
    ...
    <section className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">
          ...
          Type a sample dollar amount per service to see the breakdown.
        </p>
      </div>
                ...
                <p className="text-xs text-muted-foreground">
                  {cfg
                    ? cfg.feeType === "PERCENTAGE"
                      ? `GoStork: ${Number(cfg.percentage)}%`
                      : `GoStork: ${formatCents(Number(cfg.flatAmount) || 0)} flat`
                    : "Not configured"}
                </p>
              </div>
              <Input
                ...
                min="0"
                step="100"
                placeholder="Sample line amount"
                value={amounts[st] || ""}
                onChange={e => setAmounts(prev => ({ ...prev, [st]: e.target.value }))}
                className="flex-1"
              />
            </div>
          );
      ...
      {hasAnyAmount && (
        <div className="rounded-lg border p-4 space-y-2 bg-secondary/40">
          ...
          <div className="space-y-1 text-sm">
            {lines.filter(l => l.lineAmountCents > 0).map(l => (
              <div key={l.serviceType} className="flex justify-between text-muted-foreground">
                <span>{LINE_SERVICE_LABELS[l.serviceType]}</span>
                <span>{formatCents(l.lineAmountCents)}</span>
              </div>
            ))}
            ...
            {lines.filter(l => l.configured && l.lineAmountCents > 0).map(l => (
