...
import { CALENDAR_PROVIDERS } from "@/components/calendar/calendar-provider-icons";
...
export function CalendarSettings({ forUserId }: { forUserId?: string } = {}) {
  ...
  const authRoles: string[] = (authUser as any)?.roles || [];
  const connectionsOnly = authRoles.includes("PARENT") && authRoles.length === 1;
  ...
  useEffect(() => {
    // ?connect=true opens the provider picker; ?connect=apple deep-links straight
    // to the Apple credentials step (used by the calendar-page connect banner,
    // where the user already chose the provider).
    const connectParam = searchParams.get("connect");
    if (connectParam) {
      setConnectDialogOpen(true);
      if (connectParam === "apple") {
  ...
  const expiredConnections = connections?.filter((c) => c.tokenValid === false && (c.provider === "google" || c.provider === "microsoft")) ?? [];
  ...
  return (
    <div className="space-y-6">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap gap-2 mt-3">
                  {expiredConnections.map((conn) => {
                    ...
                    const isConnecting = isMs ? microsoftConnecting : googleConnecting;
                    return (
                      <Button
                        key={conn.id}
                        size="sm"
                      ...
                      >
                        {isConnecting ? (
                          ...
                          <providerInfo.icon className="w-3.5 h-3.5 mr-1.5" style={{ color: providerInfo.color }} />
                        ) : (
                          <Calendar className="w-3.5 h-3.5 mr-1.5" />
                        )}
                        Reconnect {conn.label || conn.email || providerInfo?.name}
                      </Button>
                    );
                  })}
                </div>
      ...
      <Card className="p-6" data-testid="scheduling-rules-section" data-onb-anchor="scheduling-rules">
        <div className="flex items-center gap-2 mb-4">
      ...
      <Card className="p-6" data-testid="date-overrides-section" data-onb-anchor="date-overrides">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
