...
export function resolveSpermDonorFields(d: any): ResolvedSpermDonorFields {
  const pd = d.profileData || {};
  return {
    age: F(d.age) || extractFromSections(pd, "Age When Donated") || extractFromSections(pd, "Age"),
    location: F(d.location) || extractFromSections(pd, "Place of Birth") || extractFromSections(pd, "Location"),
    height: F(d.height) || F(pd["Height"]) || extractFromSections(pd, "Height"),
    weight: F(d.weight) || F(pd["Weight"]) || extractFromSections(pd, "Weight"),
    hairColor: F(d.hairColor) || F(pd["Hair Color"]) || extractFromSections(pd, "Hair Color"),
    eyeColor: F(d.eyeColor) || F(pd["Eye Color"]) || extractFromSections(pd, "Eye Color"),
    ethnicity: F(d.ethnicity) || F(pd["Ethnicity"]) || extractFromSections(pd, "Ethnicity") || extractFromSections(pd, "Mother's Ethnic Origin"),
    race: F(d.race) || F(pd["Race"]) || extractFromSections(pd, "Race"),
    religion: F(d.religion) || F(pd["Religion"]) || extractFromSections(pd, "Religion") || extractFromSections(pd, "Religion Practiced"),
    education: F(d.education) || F(pd["Education Level"]) || F(pd["Education"]) || extractFromSections(pd, "Education") || extractFromSections(pd, "Education Level") || extractFromSections(pd, "Major/Area of Study"),
